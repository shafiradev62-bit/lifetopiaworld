...
function makeActionPopup(notifText: string, id: number): ActionPopupData | null {
  ...
  if (t.includes("PHANTOM CONNECTED") || t.includes("METAMASK CONNECTED") || t.includes("TRUST WALLET CONNECTED") || t.includes("WDK"))
...
export default function FarmingGame() {
  ...
  // ─── Connect WDK Wallet (Tether Wallet Development Kit — self-custodial) ───
  const connectWdk = async () => {
    setConnectingWallet("wdk");
    try {
      const { address, provider } = await connectWdkWallet();
      await _onWalletConnected(address, "solana", provider, "WDK WALLET");
    } catch (e: any) {
      console.error("[WDK]", e);
      stateRef.current.notification = { text: (e?.message || "WDK CONNECT FAILED").toUpperCase().slice(0, 40), life: 120 };
      setDs({ ...stateRef.current });
    } finally {
      setConnectingWallet(null);
    }
  ...
  useEffect(() => {
    ...
    if (savedType === "solana") {
      // WDK self-custodial wallet — address is deterministic from the stored seed
      if (hasWdkSeed()) {
        connectWdkWallet().then(({ address, provider }) => {
          if (address === savedAddr) {
            console.log("[AutoReconnect] WDK wallet silently reconnected");
            _onWalletConnected(address, "solana", provider, "WDK WALLET");
          }
        }).catch(() => {
          localStorage.removeItem("wallet_addr");
          localStorage.removeItem("wallet_type");
                  ...
                  {walletType !== null ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 8 : 12 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "0 0 2px 0" }}>
                        ...
                        <span className="gf" style={{ fontSize: 5, color: "rgba(255,255,255,0.35)" }}>SELF-CUSTODIAL</span>
                        <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.15)" }} />
                      </div>
                      {/* WDK Wallet — Tether Wallet Development Kit (self-custodial, no extension needed) */}
                      <button className="wb gf" onClick={() => connectWdk()} disabled={connectingWallet !== null}
                        style={{ fontSize: isMobile ? 6 : 8, padding: isMobile ? "8px" : "14px", background: "linear-gradient(180deg,#7B5CFF,#4B2ECF)", border: "2px solid #5C3DF0", color: "#FFE4B5", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", width: "100%", borderRadius: 999, boxShadow: "0 3px 0 #2B1590" }}>
